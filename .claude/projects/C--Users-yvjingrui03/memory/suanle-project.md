---
name: suanle-project
description: Arithmetrick（官方译名「算术魔法」）项目现状 — 四色运算DBG Roguelite，位于 E:/简单的算术魔法，68运算牌+38常规王牌+23关卡王牌，Steam商店页过审（即将上架可试玩），腾讯比赛9/12已提交，Godot 4.7
metadata: 
  node_type: memory
  type: project
...
# Arithmetrick（官方译名「算术魔法」）— 项目现状

> 命名：**正式名称 = Arithmetrick**，**算术魔法 = 官方译名**。
> 旧内部代号「简单算术魔法」已从教程文案中清除（2026-09-12），不要再使用。
...
## 卡牌状态（全完成；2026-09-12 对着配置表逐项核过）

- **运算牌 68 张**：红 18（10001-10018）/ 黄 17 / 蓝 17 / 绿 16
- **常规王牌 38 张**（商店可售 = 白 2 + 红黄蓝绿各 9）
  · 另有 9 张事件专属（5xxx，标 `SHOP_EXCLUDE=true`，商店不出售），故 `WildConfig` 总计 47
  · **口径坑**：对外材料统一写 **38**（用户 2026-09-12 选定）
- **关卡王牌（试炼）23 张**（9001-9023，见 `LevelWildConfig`）
- **魔法** 约 20 余种（`MagicConfig` 共 27 条，含若干开局加成项）
- 绿色套牌代码完整但**游戏内封锁**（仅图鉴可预览整套设计）

> 旧数字（红黄蓝各 17 / 绿 15、常规 36、关卡 22）**已作废** —— 那是更早的快照，
> 之后绿牌补齐、事件专属牌与第 23 张关卡王牌才陆续加入。
